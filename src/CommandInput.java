...
public class CommandInput {
    ...
    public void save() {    	    
        ...
        try {       //JSON Conversion
        	PrintWriter pwDefault = new PrintWriter(new File("Collection.json"));
    ...
    public void insert_key(Scanner scanner,String value) {
        long keys;
        keys = Keys.keys();
        ...
        if (CCollection.collection.size() != 0) {
            ...
            while (a <= CCollection.collection.size()) {
                if (CCollection.collection.get(key) != null) {
                    System.out.println("Key value: " + key + " | Name: " + CCollection.collection.get(key).getName() + " | Coordinates: (" + CCollection.collection.get(key).getCordinatesX() + ";" + CCollection.collection.get(key).getCordinatesY() + ")" + " | Health Points: " + CCollection.collection.get(key).getHealth() + " | Weapon Type: " + CCollection.collection.get(key).getWeapon() + " | Melee Weapon Type: " + CCollection.collection.get(key).getMeleeWeapon() + " | Chapter Description (" + CCollection.collection.get(key).getChapter().getName() + ";" + CCollection.collection.get(key).getChapter().getParentLegion() + ")");
                    key++;
                    a++;
                } else key++;
            }
                ...
                if (CCollection.collection.get(i) != null) {
                    ...
                    while (j <= CCollection.collection.size()) {
                        if (CCollection.collection.get(j) != null) {
                            if (CCollection.collection.get(i).getChapter().getName() == CCollection.collection.get(j).getChapter().getName()) {
                                x.add(j);
                                j++;
                            } else j++;
                        } else j++;
                    ...
                    if (x.contains(i) == false) {
                        System.out.println(CCollection.collection.get(i).getChapter().getName());
                    }
                    i++;
                    a++;
                } else i++;
    ...
    public void replace(Scanner Replace,String value) {
        if (CCollection.collection.size() != 0) {
            long key;
            ...
            if (key != 0) {
                ...
                if (i == 1) {
                    ...
                    if (name2.length() < CCollection.collection.get(key).getName().length()) {
                        SpaceMarine rep = new SpaceMarine(name2, new Coordinates(CCollection.collection.get(key).getCordinatesX(), CCollection.collection.get(key).getCordinatesY()), CCollection.collection.get(key).getHealth(), CCollection.collection.get(key).getHeight(), CCollection.collection.get(key).getWeapon(),CCollection.collection.get(key).getMeleeWeapon(), CCollection.collection.get(key).getChapter(), CCollection.collection.get(key).getId());
                ...
                if (i == 2) {
                    ...
                    {
                        SpaceMarine rep = new SpaceMarine(CCollection.collection.get(key).getName(), new Coordinates(x2, y2), CCollection.collection.get(key).getHealth(), CCollection.collection.get(key).getHeight(), CCollection.collection.get(key).getWeapon(),CCollection.collection.get(key).getMeleeWeapon(), CCollection.collection.get(key).getChapter(), CCollection.collection.get(key).getId());
                        CCollection.collection.put(key, rep);
                    }
                }
                if (i == 3) {
                    ...
                    {
                        SpaceMarine rep = new SpaceMarine(CCollection.collection.get(key).getName(), new Coordinates(CCollection.collection.get(key).getCordinatesX(), CCollection.collection.get(key).getCordinatesY()), hel, CCollection.collection.get(key).getHeight(), CCollection.collection.get(key).getWeapon(),CCollection.collection.get(key).getMeleeWeapon(), CCollection.collection.get(key).getChapter(), CCollection.collection.get(key).getId());
                        CCollection.collection.put(key, rep);
                    }
                }
            } else {
    ...
    public void script(Scanner commandReader) {
        int sk=0;
        int c=0;
    ...
    public void raw_start() throws FileNotFoundException{
        ...
        public void insert_key2(Scanner scanner2){
            long keys;
            keys = Keys.keys();
            SpaceMarine space = Script.Scan2(scanner2);
            if (space != null) {
                CCollection.collection.put(keys, space);
            }
        }
        public void insert_key3(Scanner scan,String filik,int sk) throws FileNotFoundException {
        Main.loadJson(filik);
        }
        public CommandInput(String com){
        for (int i=1;i<commandArray.length;i++) {
            if (com.equals(commandArray[i])){
             com=commandArray[i];
            }
        }
        }
